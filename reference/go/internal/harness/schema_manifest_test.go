...
func TestStructuralCBORSchemaManifestV1(t *testing.T) {
  ...
  if len(m.RootSet) != 10 || len(m.Event) != 7 || len(m.Gate) != 9 { t.Fatalf("manifest does not cover all members: root=%d event=%d gate=%d", len(m.RootSet), len(m.Event), len(m.Gate)) }
  for _, field := range []string{"owners", "interfaces", "applicability", "required_evidence", "activation_ref", "status", "evidence_digest", "evidence_retention_valid"} {
    if _, ok := m.Gate[field]; !ok { t.Fatalf("manifest missing gate_record.%s", field) }
  }
}
