...
- [x] 3.1 **S02-RH-W03** Publish bounded structural fixtures and expected reports, compare parsed Rust and Go outputs byte-for-byte, and include mutation/reset/replay vectors. Verification: `pwsh -NoProfile -File scripts/compare-reference-harness.ps1 -EvidencePath (Join-Path $env:TEMP 'mcb-structural-candidate.json')`.
