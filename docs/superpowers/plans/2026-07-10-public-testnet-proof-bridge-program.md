...
- Every closure review binds one `ProgramSnapshotV1`. A scoped input change invalidates the affected review and all dependent artifacts.
- Network identity, official-root, unknown or semantic runtime, finality-profile, or public-reset drift returns to `PBT-S02` and invalidates dependent circuits, setup, deployment, and public receipts. Catalog or family drift returns to `PBT-S03`; claim or registry semantics to `PBT-S04`; circuit or ceremony-verifier drift to `PBT-S07`; and human policy or transcript drift to `PBT-S08`. Only endpoint-only drift or a fingerprint transition already authorized by the frozen runtime policy may re-enter at `PBT-S11` or the affected `PBT-S12` package.
...
The reviewed implementation snapshot and later review artifacts are joined by
`ClosureEnvelopeV1`. The envelope permits only review requests and results,
...
node and graph materialization updates, wiki log events, inventories, seal receipts, and,
...
affected readers. The envelope's typed delta manifest enumerates every permitted
...
| `PBT-S00-W02` | Define `ProgramPlanV1` and publish the canonical plan | `PBT-S00-W01` `[implementation-complete]` | `program/schemas/program-plan-v1.schema.json`; `program/plans/public-testnet-livepass-v2.json` | Schema, id, count, dependency-state, cycle, and artifact-path tests prove 14 sprints and 100 unique packages. |
| `PBT-S00-W03` | Define brokered `ProgramEventV1` and deterministic reduction | `PBT-S00-W02` `[implementation-complete]` | committed native service source; reproducible build and elevated provisioning receipts; controller identity and capabilities; event reducer | Two unelevated clean-clone builds of committed source reproduce before the installer accepts the binary; elevation never runs build code. Restricted tokens cannot mutate controller or canonical Git storage. Restart preserves identity and durable head; cross-attempt capabilities and invalid event transitions reject. |
| `PBT-S00-W04` | Implement leases, attempts, retry, crash recovery, and resume | `PBT-S00-W03` `[implementation-complete]` | attempt, renewal, release, and fencing functions; recovery fixtures | Expiry during a command, renewal loss, clock jump, late writer, interrupted command, unknown submission, retry, remediation, and resume suites pass. |
| `PBT-S00-W05` | Implement the universal command supervisor and package entrypoint | `PBT-S00-W03` `[implementation-complete]`; `PBT-S00-W04` `[implementation-complete]` | command module; authenticated one-shot canonical seed; independent-clone transaction module; bounded pack quarantine; `CommandRecordV2`; package entrypoint | The broker verifies the committed W01-W05 lineage before seeding canonical Git. Start-before-launch, byte-preserving streams, capability and clone containment, host/WSL/container cleanup, hostile-pack quarantine, broker import, lease fencing, required output, executable hash, and prepublication secret tests pass. |
| `PBT-S00-W06` | Validate runlogs, inventories, and redaction receipts | `PBT-S00-W05` `[implementation-complete]` | runlog validator; scanner and ruleset; repaired runlog schemas | Cross-file negative fixtures reject traversal, omissions, hash drift, duplicate roles, false threads, forbidden fields, secrets, and incomplete terminal state. |
...
| `PBT-S00-W08` | Harden repository integration and remote publication | `PBT-S00-W04` `[implementation-complete]`; `PBT-S00-W07` `[implementation-complete]` | concurrency, remote lease, qualified opaque Git credential handle, signed `RemoteConfirmationBundleV1`, and atomic publication module | Tests reject dirty, out-of-scope, stale-fence, stale-remote, force-push, concurrent, prompted, revoked-credential, partial-bundle, blind-repush, incomplete-predecessor, and secret-leak cases. Signed command records and exact streams, full-bundle import, and remote SHA reproduction pass. |
| `PBT-S00-W09` | Compile bounded Grok sprint packets and environment receipts | `PBT-S00-W02` `[implementation-complete]`; `PBT-S00-W05` `[implementation-complete]`; `PBT-S00-W07` `[implementation-complete]` | XML packet template, compiler, environment probe | Packet schema, prompt hash, permission, model, exact snapshot, allowed paths, package-entrypoint routing, and no-secret environment tests pass. |
| `PBT-S00-W10` | Run isolated Codex audits and persona councils | `PBT-S00-W05` `[implementation-complete]`; `PBT-S00-W07` `[implementation-complete]`; `PBT-S00-W09` `[implementation-complete]` | supervised audit and reader runners; review and typed closure-envelope schemas | Full-clone runner tests prove actor provenance, separate scratch, stream capture, thread extraction, timeout cleanup, immutable rounds, one-target equality, zero-count rules, and rejection of hostile semantic changes inside allowed closure paths. |
...
| `PBT-S00-W12` | Publish the base `GateRosterV2`, CI, smoke run, and closure review | `PBT-S00-W01` `[implementation-complete]`; `PBT-S00-W02` `[implementation-complete]`; `PBT-S00-W03` `[implementation-complete]`; `PBT-S00-W04` `[implementation-complete]`; `PBT-S00-W05` `[implementation-complete]`; `PBT-S00-W06` `[implementation-complete]`; `PBT-S00-W07` `[implementation-complete]`; `PBT-S00-W08` `[implementation-complete]`; `PBT-S00-W09` `[implementation-complete]`; `PBT-S00-W10` `[implementation-complete]`; `PBT-S00-W11` `[implementation-complete]` | `protocol/gate-roster-v2.schema.json`; `protocol/gate-evaluation-v1.schema.json`; base roster; CI workflow; smoke runlog; closure envelope | The immutable base roster starts unresolved and is explicitly execution/classification-ineligible; append-only evaluations carry current evidence. All control suites pass one candidate snapshot; the typed envelope binds 0/0/0 reviews and a byte-identical OpenSpec archive; a separate post-push receipt confirms the remote SHA before successor readiness. |
...
| `PBT-S01-W05` | Run a fresh snapshot-bound reader round | `PBT-S01-W04` `[implementation-complete]` | immutable proof, consensus, operator, security, and Codex artifacts | Every reader names one `ProgramSnapshotV1`; any nonzero finding creates a new target and repeats all scoped readers. |
...
| `PBT-S03-W08` | Run independent catalog council and activate the corpus | `PBT-S03-W05` `[implementation-complete]`; `PBT-S03-W06` `[implementation-complete]`; `PBT-S03-W07` `[implementation-complete]`; `PBT-S02-W09` `[closed]` | `predicate-catalogs/admission-receipt-v1.json`; reader artifacts | Proof, source, consensus, and operator readers report 0/0/0 for the same corpus digest and demonstrated public profiles. |
...
| `PBT-S07-W08` | Freeze circuits and pass security review | `PBT-S07-W06` `[implementation-complete]`; `PBT-S07-W07` `[implementation-complete]` | `ceremony/freeze/circuit-freeze-v1.json`; review round | Exact constraints, public-input order, committed-wire indices, blinding ownership, compiler, tools, KZG binding, profiles, artifacts, and invalidation rules freeze at one digest; readers report 0/0/0. |
...
| Package | Work package | Depends on | Primary artifacts | Exit or stop evidence |
| --- | --- | --- | --- | --- |
| `PBT-S08-W01` | Validate frozen inputs, ceremony modes, schedules, and human enrollment | `PBT-S07-W08` `[closed]` | `ceremony/runs/policy-v1.json`; `CeremonyBeaconScheduleV1`; historical-qualification references; enrollment receipts | Every setup tuple selects exactly one mode. `new-or-update` freezes a distinct future beacon, domain, close point, minimum count, independence tests, contributor keys, publication channels, final-head acknowledgements, and public anchor. `historical-qualified` pins the original ceremony evidence and contributor policy and creates no retroactive beacon. Exact freeze digest and independently controlled environments validate; missing historical evidence or a new-ceremony shortage records `waiting-external` or blocks according to policy. |
...
| `PBT-S10-W07` | Run predeployment council and artifact freeze | `PBT-S10-W01` `[implementation-complete]`; `PBT-S10-W02` `[implementation-complete]`; `PBT-S10-W03` `[implementation-complete]`; `PBT-S10-W04` `[implementation-complete]`; `PBT-S10-W05` `[implementation-complete]`; `PBT-S10-W06` `[implementation-complete]` | `program/snapshots/pbt-s10-predeployment/` | Proof, consensus, operator, security, and Codex readers report 0/0/0 against one complete frozen snapshot. |
...
| `PBT-S13-W03` | Run fresh proof, consensus, operator, and security council | `PBT-S13-W01` `[implementation-complete]` | four immutable reader reports | Every supervised reader names the same snapshot, checks its declared scope, and reports 0/0/0. A finding returns to its earliest owning package. |
...
- [ ] The controller reduces one valid append-only history covering all 100 packages.
...
- [ ] Fresh Codex, proof, consensus, operator, and security reviews report 0/0/0 for the final public snapshot.
