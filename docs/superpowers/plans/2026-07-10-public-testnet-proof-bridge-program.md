...
| `PBT-S13-W02` | Run fresh isolated Codex audit | `PBT-S13-W01` `[implementation-complete]` | immutable Codex request, JSONL, stderr, response, disposition | Codex inspects a full disposable clone of the committed snapshot and emits a complete immutable audit without editing the clone or leaving scratch residue. Package completion depends on artifact integrity and scope, not finding counts or verdict. Every finding proceeds unchanged to W04. |
